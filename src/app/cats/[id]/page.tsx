        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-4">
            <div className="relative aspect-square rounded-3xl overflow-hidden shadow-2xl bg-muted border border-border">
              ...
              <Badge className={`absolute top-4 right-4 backdrop-blur-sm text-lg px-4 py-2 ${cat.status === 'ADOPTED' ? 'bg-pink-600/90' : 'bg-primary/90'}`}>
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link href={`/cats/${cat.id}/apply`} className={`flex-1 ${cat.status === 'ADOPTED' ? 'pointer-events-none' : ''}`}>
                <Button
                  size="lg"
                  className={`w-full h-14 text-lg rounded-full ${cat.status === 'ADOPTED'
                ...
                >
                  ...
                  {cat.status === 'ADOPTED' ? 'Already Adopted' : 'Apply for Adoption'}
                </Button>
              </Link>
              <DonationModal
                catId={cat.id}
